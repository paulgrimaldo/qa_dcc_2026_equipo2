#!/usr/bin/env bash
# Script de Pruebas de Entradas Inválidas para Pet Clinic REST
# 
# Escenario Q3: Robustez ante IDs inválidos en /pets/{id} (Robustness / Error Handling)
# 
# Este script atiende al escenario Q3 probando el manejo de entradas inválidas.
#
# Estímulo: se solicita GET /pets/{id} con valores inválidos (e.g., -1, 0, 999999, abc)
...
for id in "${INVALID_IDS[@]}"; do
    ...
    code=$(curl -s -o "${OUTPUT_DIR}/invalid_pet_${id}.json" -w "%{http_code}" "${BASE_URL}/pets/${id}")
